...
(function () {
  ...
  function renderCard(piece, idx) {
    ...
    return `
      <article class="card" data-plate="${piece.plate}" data-cat="${piece.category}" data-century="${getEarliestCentury(piece.period)}" tabindex="0" role="button" aria-label="${esc(piece.title)} — Plate ${piece.plate}">
        ${imgPart}
        <div class="card-body">
  ...
  function openModal(piece) {
    ...
    state._returnFocus = document.activeElement;
  ...
  function closeModal() {
    ...
    if (state._returnFocus) {
      state._returnFocus.focus();
      state._returnFocus = null;
    }
  }
